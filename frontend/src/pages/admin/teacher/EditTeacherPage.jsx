...
import { EditTeacher } from '../../../components/forms'
...
const EditTeacherPage = () => {
  ...
  const uploadImage = async options => {
    ...
    try {
      await api.post(`/users/teachers/picture/${id}`, fmData, config)
      onSuccess('Ok')
      setFetch(true)
    } catch (err) {
          ...
          <div className="flex flex-col lg:flex-row w-full text-white items-center justify-start py-5 space-y-4 lg:space-y-0 overflow-auto">
            <div className="flex flex-col space-y-5 w-full lg:w-1/3 h-full items-center justify-center">
              {teacher
                ? (
                <>
                  <Image src={teacher.picture} className="h-[80%] w-[60%]" fallback={FallBack}/>
                  <ImgCrop rotate>
                    <Upload {...uploadProps}>
